...
class BenchArgs:
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--repeat-prefill",
            type=int,
            default=BenchArgs.repeat_prefill,
            help="Number of times to repeat the prefill step.",
        )
...
def latency_test_run_once(
    ...
    for i in range(repeat_prefill):
        ...
        rank_print(
            f"Prefill. latency {i}: {prefill_latency:6.5f} s, throughput: {throughput:9.2f} token/s"
        )
...
def latency_test(
    ...
    for bs, il, ol in itertools.product(
        ...
        ret = latency_test_run_once(
            ...
            repeat_prefill=bench_args.repeat_prefill,
        )
        gc.enable()
        gc.collect()
        if ret is not None:
